...
def get_study_group(request):
    ...
    my_study_group = []
    for deck in decks:
        study_group = StudyGroup.objects.get(deck_id=deck.deck_id)
...
def get_chats(request):
    ...
    chats = Chat.objects.filter((Q(group_id=group_id)) & (Q(from_user=friend_name) | Q(to_user=friend_name))).order_by(
        'c_time')
    if chats.exists():
        all_chats = []
        for chat in chats:
            ...
            all_chats.append(
                {'user_name': user.user_name, 'user_avatar': author_avatar, 'content': chat.content,
                 'c_time': chat.c_time.strftime('%Y-%m-%d %H:%M:%S')}
            )
        ret['data'] = all_chats
    else:
        ret['status'] = False
